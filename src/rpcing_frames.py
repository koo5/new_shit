...
class ClientFrame(object):
	...
	def maybe_draw(s):
		...
		if must_recollect or s.lines.dirty or s.tags.dirty or s.must_redraw:  # or not args.rpc:
			log("drawing %s, must_recollect = %s, lines.dirty = %s, must_redraw = %s", s.counterpart, must_recollect,
			    s.lines.dirty, s.must_redraw)
			s.draw()
			s.must_redraw = False
		...
		for row, line in enumerate(s.lines):
			...
			for col, graphics in line.graphics.items():
				...
				for graphic in graphics:
					...
					pygame.draw.line(surf, c, (x, y1), (x, y1 - line.font.height / 5), width)
					pygame.draw.line(surf, c, (x, y2), (x, y2 + line.font.height / 5), width)
	...
	if CURSES:
		...
		def curses_draw_line(s, win, line):
			for col, char in enumerate(line):
				...
				except c.error:  # addch throws an error to indicate last cell
...
class Editor(ClientFrame):
	...
	def do_post_render_move_cursor(s):
		...
		if m.node is not None or m.chars != 0:
			m.node = None
			m.chars = 0
...
class SidebarFrame(ClientFrame):
	...
	def sdl_draw_items(s, surface):
		...
		for i, item in enumerate(s.get_items()):
			...
			for j in lines_to_draw:
				s.sdl_draw_line(surface, *j)
...
def collidepoint(r, pos):
	x, y = pos
	return x >= r[0] and r[0] + r[2] > r[1] <= y < r[1] + r[3]
